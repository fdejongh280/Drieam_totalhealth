 ...
 // ['Rijks museum', 'Museumstraat 1, 1071 XX, Amsterdam', 'Rijks@gmail.com', 43],
      ...
      if (status === google.maps.GeocoderStatus.OK) {
        ...
        for (i = 0; i < locations.length; i++) {
          ...
          if (distance <= radiusmetric) {
            ...
            in_area.push([locations[i][0], locations[i][3], locations[i][6], (distance / 1000).toFixed(1)]); // distance/1000 van meters -> kilometers
          } else {
  ...
  function resultstotal(resultstotal, radius, search, circlearea) {
    ...
    for (i = 0; i < in_area.length; i++) {
      ...
      $('main ol').append('<li class = "therapist" value = "'+locations[i][7]+'">' +
    ...
    $('main ol .therapist').on('click', function(){
     ...
     var alumni = searchForCorrespondingAlumni($(this).val());
  ...
  function fillAlumniPage(alumni)
  {
    ...
    {
      $('.alumnicontainer .content').append('<button id = "editText">Text bewerken</button>')
      $('#editText').on('click', function(){
        $('.alumnicontainer .content .info').attr('contentEditable',true);
        $('.alumnicontainer .content').append('<button id = "saveChanges">Bewerking opslaan</button>')
      })
    }
}
